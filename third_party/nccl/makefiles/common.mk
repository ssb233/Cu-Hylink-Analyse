...
########## Primitive trace experiment ##########
PRIMITIVE_TRACE_FENCE_SCOPE ?= gpu
ifneq ($(filter-out sys gpu,$(PRIMITIVE_TRACE_FENCE_SCOPE)),)
  $(error PRIMITIVE_TRACE_FENCE_SCOPE must be sys or gpu)
endif
PRIMITIVE_TRACE_FENCE_SCOPE_SYS := $(if $(filter sys,$(PRIMITIVE_TRACE_FENCE_SCOPE)),1,0)
NVCUFLAGS += -DNCCL_EXPERIMENT_FENCE_SCOPE_SYS=$(PRIMITIVE_TRACE_FENCE_SCOPE_SYS)
NVCUFLAGS_SYM += -DNCCL_EXPERIMENT_FENCE_SCOPE_SYS=$(PRIMITIVE_TRACE_FENCE_SCOPE_SYS)
PRIMITIVE_TRACE ?= 0
PRIMITIVE_TRACE_KIND ?= post
PRIMITIVE_TRACE_KIND_ID := $(if $(filter wait,$(PRIMITIVE_TRACE_KIND)),1,$(if $(filter fence,$(PRIMITIVE_TRACE_KIND)),2,$(if $(filter store,$(PRIMITIVE_TRACE_KIND)),3,$(if $(filter post,$(PRIMITIVE_TRACE_KIND)),4,$(if $(filter copy,$(PRIMITIVE_TRACE_KIND)),5,0)))))
ifneq ($(PRIMITIVE_TRACE),0)
  CXXFLAGS += -DNCCL_EXPERIMENT_PRIMITIVE_TRACE=1
  NVCUFLAGS += -DNCCL_EXPERIMENT_PRIMITIVE_TRACE=1
  NVCUFLAGS += -DNCCL_EXPERIMENT_PRIMITIVE_TRACE_KIND=$(PRIMITIVE_TRACE_KIND_ID)
  NVCUFLAGS_SYM += -DNCCL_EXPERIMENT_PRIMITIVE_TRACE=1
  NVCUFLAGS_SYM += -DNCCL_EXPERIMENT_PRIMITIVE_TRACE_KIND=$(PRIMITIVE_TRACE_KIND_ID)
endif
########## Primitive trace experiment ##########
