...
usage() {
  cat <<'EOF'
Usage: run_q2_stream_count_matrix.sh [options]

Run the Q2 eligible-stream-count diagnostic matrix for four-GPU allpairs D2D
traffic. The fixed stream mapping is:
  1 stream/source             all three outgoing edges share one stream
  2 streams/source            queue positions 0/2 share, position 1 is separate
  3 streams/source            one stream per directed edge
...
All Q2 cases use streamDependency=none. D2D warmup is fixed at 10 iterations.
EOF
}
...
IFS=',' read -r -a stream_count_array <<< "${stream_counts}"
IFS=',' read -r -a repeats_array <<< "${repeats_list}"
IFS=',' read -r -a scenario_array <<< "${scenarios}"
[[ "${#stream_count_array[@]}" -gt 0 && -n "${stream_count_array[0]}" ]] || {
  echo "ERROR: --streamCounts cannot be empty" >&2
  exit 2
}
...
for index in "${!stream_count_array[@]}"; do
  stream_count_array[index]="${stream_count_array[index]//[[:space:]]/}"
  case "${stream_count_array[index]}" in
    1|2|3) ;;
    *)
      echo "ERROR: stream counts must be 1, 2, or 3: ${stream_count_array[index]}" >&2
      exit 2
      ;;
  esac
done
...
if [[ -z "${output_root}" ]]; then
  ...
  output_root="${repo_root}/doc/results/gpu-contention/queue-phase-diagnostic/q2-stream-count/${timestamp}-$$"
fi
mkdir -p "${output_root}"
...

{
  ...
  printf 'device_list=%s\nedge_order=%s\nstream_counts=%s\nrepeats_list=%s\nscenarios=%s\nruns=%s\nsize=%s\n' \
    "${device_list}" "${edge_order}" "${stream_counts}" "${repeats_list}" \
    "${scenarios}" "${runs}" "${size}"
  printf 'stream_dependency=none\n'
  printf '\n### command_line ###\n'
...
printf 'streamsPerSource,repeats,repetition,scenario,size,d2dAggregateGBps,backgroundAggregateGBps,d2dExit,backgroundExit,status,d2dJson,backgroundJson,d2dLog,backgroundLog\n' > "${summary_file}"
...
run_case() {
  local streams_per_source="$1"
  local repeats="$2"
  local repetition="$3"
  local scenario="$4"
  local direction="none"
  local case_dir="${output_root}/streams-${streams_per_source}/repeats-${repeats}/repetition-${repetition}/${scenario}"
  ...
  {
    printf 'streamsPerSource=%s edgeOrder=%s repeats=%s repetition=%s scenario=%s\n' \
      "${streams_per_source}" "${edge_order}" "${repeats}" "${repetition}" \
      "${scenario}"
    printf 'd2d_command='
    printf '%q ' "${d2d_cmd[@]}"
    printf '\n'
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "${streams_per_source}" "${repeats}" "${repetition}" "${scenario}" \
  ...
  if [[ "${status}" != "pass" ]]; then
    echo "ERROR: failed case streams=${streams_per_source} repeats=${repeats} scenario=${scenario}; see ${case_dir}" >&2
    return 1
  fi
  echo "PASS: streamsPerSource=${streams_per_source} repeats=${repeats} scenario=${scenario} d2dAggregateGBps=${d2d_gbps} backgroundAggregateGBps=${background_gbps}"
}

for streams_per_source in "${stream_count_array[@]}"; do
  for repeats in "${repeats_array[@]}"; do
    for repetition in $(seq 1 "${runs}"); do
      for scenario in "${scenario_array[@]}"; do
        run_case "${streams_per_source}" "${repeats}" "${repetition}" \
          "${scenario}"
      done
    done
  done
...
echo "Completed Q2 stream-count matrix: ${output_root}"
