...
offsets_list="0,0,0,0:0,250,500,750:0,500,1000,1500:0,1000,2000,3000:0,2000,4000,6000:0,4000,8000,12000"
repeats_list="20,300"
...
usage() {
  cat <<'EOF'
Usage: run_q4_source_offset_matrix.sh [options]

Run the Q4 source-phase-offset diagnostic matrix for four-GPU allpairs D2D
traffic. Each source uses one stream and receives one device-side delay before
the measured batch. Offset values are in microseconds and are source-indexed.
Use ':' between offset vectors.

Options:
  --deviceList=0,1,2,3
  --edgeOrder=source-major
  --offsetsList=0,0,0,0:0,250,500,750:0,500,1000,1500:0,1000,2000,3000:0,2000,4000,6000:0,4000,8000,12000
  --repeatsList=20,300
  --scenarios=none,d2h-all
...
All Q4 cases use streamMode=per-source, streamsPerSource=1,
streamDependency=none, and edgePermutation=identity. D2D warmup is fixed at
10 iterations. The source delay kernel is not included in the per-source CUDA
event bandwidth interval; its actual timeline position is checked in Nsight.
EOF
}
...
[[ "${edge_order}" == "source-major" ]] || {
  echo "ERROR: Q4 requires --edgeOrder=source-major" >&2
  exit 2
}

IFS=',' read -r -a device_array <<< "${device_list}"
[[ "${#device_array[@]}" -eq 4 ]] || {
  echo "ERROR: Q4 requires exactly four devices" >&2
  exit 2
}
...
IFS=':' read -r -a offset_array <<< "${offsets_list}"
IFS=',' read -r -a repeats_array <<< "${repeats_list}"
IFS=',' read -r -a scenario_array <<< "${scenarios}"
[[ "${#offset_array[@]}" -gt 0 && -n "${offset_array[0]}" ]] || {
  echo "ERROR: --offsetsList cannot be empty" >&2
  exit 2
}
...
for index in "${!offset_array[@]}"; do
  offset_array[index]="${offset_array[index]//[[:space:]]/}"
  IFS=',' read -r -a offset_values <<< "${offset_array[index]}"
  [[ "${#offset_values[@]}" -eq "${#device_array[@]}" ]] || {
    echo "ERROR: every offset vector must contain four values: ${offset_array[index]}" >&2
    exit 2
  }
  canonical=""
  for value in "${offset_values[@]}"; do
    [[ "${value}" =~ ^[0-9]+$ ]] || {
      echo "ERROR: source offsets must be non-negative integers: ${offset_array[index]}" >&2
      exit 2
    }
    if [[ -n "${canonical}" ]]; then canonical+=","; fi
    canonical+="${value}"
  done
  offset_array[index]="${canonical}"
done
...
if [[ -z "${output_root}" ]]; then
  ...
  output_root="${repo_root}/doc/results/gpu-contention/queue-phase-diagnostic/q4-source-offset/${timestamp}-$$"
fi
mkdir -p "${output_root}"
...

{
  ...
  printf 'device_list=%s\nedge_order=%s\noffsets_list=%s\nrepeats_list=%s\nscenarios=%s\nruns=%s\nsize=%s\n' \
    "${device_list}" "${edge_order}" "${offsets_list}" "${repeats_list}" \
    "${scenarios}" "${runs}" "${size}"
...
printf 'offsetId,offsetsUs,repeats,repetition,scenario,size,d2dAggregateGBps,backgroundAggregateGBps,d2dExit,backgroundExit,status,d2dJson,backgroundJson,d2dLog,backgroundLog\n' > "${summary_file}"
...
run_case() {
  ...
  local offset_id="offset-${offset_index}"
  local case_dir="${output_root}/${offset_id}/repeats-${repeats}/repetition-${repetition}/${scenario}"
  ...
  d2d_cmd=(
    ...
    "--sourceOffsetsUs=${offsets}"
    "--size=${size}"
    "--repeats=${repeats}"
    "--output=${d2d_json}"
  )
  {
    printf 'offsetId=%s offsetsUs=%s repeats=%s repetition=%s scenario=%s\n' \
      "${offset_id}" "${offsets}" "${repeats}" "${repetition}" "${scenario}"
    printf 'd2d_command='
    printf '%q ' "${d2d_cmd[@]}"
    printf '\n'
  ...
  printf '%s,"%s",%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "${offset_id}" "${offsets}" "${repeats}" "${repetition}" \
  ...
  if [[ "${status}" != "pass" ]]; then
    echo "ERROR: failed case offsets=${offsets} repeats=${repeats} scenario=${scenario}; see ${case_dir}" >&2
    return 1
  fi
  echo "PASS: offsets=${offsets} repeats=${repeats} scenario=${scenario} d2dAggregateGBps=${d2d_gbps} backgroundAggregateGBps=${background_gbps}"
}

for offset_index in "${!offset_array[@]}"; do
  offsets="${offset_array[${offset_index}]}"
  for repeats in "${repeats_array[@]}"; do
    for repetition in $(seq 1 "${runs}"); do
      for scenario in "${scenario_array[@]}"; do
        run_case "${offset_index}" "${offsets}" "${repeats}" \
          "${repetition}" "${scenario}"
      done
    done
  done
...
echo "Completed Q4 source-offset matrix: ${output_root}"
