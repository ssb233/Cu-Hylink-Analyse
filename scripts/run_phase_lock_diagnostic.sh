...
device_list="0,1,2,3"
edge_orders="source-major,destination-major"
stream_modes="per-source,per-edge"
repeats_list="20,300"
...
usage() {
  cat <<'EOF'
Usage: run_phase_lock_diagnostic.sh [options]

Run the edge-order and stream-topology diagnostic matrix for four-GPU
allpairs D2D traffic. The d2h-all scenario runs one background D2H worker
on every listed GPU.

Options:
  --deviceList=0,1,2,3
  --edgeOrders=source-major,destination-major
  --streamModes=per-source,per-edge
  --repeatsList=20,300
  --scenarios=none,d2h-all
...
[[ "${#edge_order_array[@]}" -gt 0 && -n "${edge_order_array[0]}" ]] || {
  echo "ERROR: --edgeOrders cannot be empty" >&2
  exit 2
}
[[ "${#stream_mode_array[@]}" -gt 0 && -n "${stream_mode_array[0]}" ]] || {
  echo "ERROR: --streamModes cannot be empty" >&2
  exit 2
}
...
for index in "${!edge_order_array[@]}"; do
  edge_order_array[index]="${edge_order_array[index]//[[:space:]]/}"
  case "${edge_order_array[index]}" in
    source-major|destination-major) ;;
    *) echo "ERROR: unsupported edge order: ${edge_order_array[index]}" >&2; exit 2 ;;
  esac
done
for index in "${!stream_mode_array[@]}"; do
  stream_mode_array[index]="${stream_mode_array[index]//[[:space:]]/}"
  case "${stream_mode_array[index]}" in
    per-source|per-edge) ;;
    *) echo "ERROR: unsupported stream mode: ${stream_mode_array[index]}" >&2; exit 2 ;;
  esac
done
...
for index in "${!scenario_array[@]}"; do
  ...
  case "${scenario_array[index]}" in
    none|d2h-all) ;;
    *) echo "ERROR: unsupported scenario: ${scenario_array[index]}" >&2; exit 2 ;;
  esac
done

if [[ -z "${output_root}" ]]; then
  timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
  output_root="${repo_root}/doc/results/gpu-contention/phase-lock-diagnostic/${timestamp}-$$"
fi
mkdir -p "${output_root}"
...

{
  ...
  printf 'device_list=%s\nedge_orders=%s\nstream_modes=%s\nrepeats_list=%s\nscenarios=%s\nruns=%s\nsize=%s\n' \
    "${device_list}" "${edge_orders}" "${stream_modes}" "${repeats_list}" \
    "${scenarios}" "${runs}" "${size}"
  printf '\n### command_line ###\n'
...
printf 'edgeOrder,streamMode,repeats,repetition,scenario,size,d2dAggregateGBps,backgroundAggregateGBps,d2dExit,backgroundExit,status,d2dJson,backgroundJson,d2dLog,backgroundLog\n' > "${summary_file}"
...
run_case() {
  ...
  local case_dir="${output_root}/order-${edge_order}/stream-${stream_mode}/repeats-${repeats}/repetition-${repetition}/${scenario}"
  ...
  case "${scenario}" in
    ...
    *) echo "ERROR: unsupported scenario: ${scenario}" >&2; return 2 ;;
  esac
  ...
  {
    printf 'edgeOrder=%s streamMode=%s repeats=%s repetition=%s scenario=%s\n' \
      "${edge_order}" "${stream_mode}" "${repeats}" "${repetition}" "${scenario}"
    printf 'd2d_command='
    printf '%q ' "${d2d_cmd[@]}"
    printf '\n'
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "${edge_order}" "${stream_mode}" "${repeats}" "${repetition}" "${scenario}" \
    "${size}" "${d2d_gbps}" "${background_gbps}" "${d2d_rc}" \
    "${background_case_rc}" "${status}" "${d2d_json}" "${background_json}" \
    "${d2d_log}" "${background_log_entry}" >> "${summary_file}"

  if [[ "${status}" != "pass" ]]; then
    echo "ERROR: failed case order=${edge_order} stream=${stream_mode} repeats=${repeats} scenario=${scenario}; see ${case_dir}" >&2
    return 1
  fi
  echo "PASS: order=${edge_order} stream=${stream_mode} repeats=${repeats} scenario=${scenario} d2dAggregateGBps=${d2d_gbps} backgroundAggregateGBps=${background_gbps}"
}
    ...
    for repeats in "${repeats_array[@]}"; do
      for repetition in $(seq 1 "${runs}"); do
        for scenario in "${scenario_array[@]}"; do
          run_case "${edge_order}" "${stream_mode}" "${repeats}" \
            "${repetition}" "${scenario}"
        done
      done
    done
...
echo "Completed phase-lock diagnostic matrix: ${output_root}"
